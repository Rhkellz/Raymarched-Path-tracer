...
public class FPSCounter : MonoBehaviour
{
    ...
    public float logDuration = 10f; // seconds
    ...
    void Update()
    {
        ...
        if (elapsedTime < logDuration) {
            ...
            if (logWrite) {
                string path = System.IO.Path.Combine(Application.persistentDataPath, "FPSLog.csv");
                using (var sw = new System.IO.StreamWriter(path, true)) {
                    sw.WriteLine($"{elapsedTime:F2},{currentFPS:F2}");
                }
            }
        }
    }
